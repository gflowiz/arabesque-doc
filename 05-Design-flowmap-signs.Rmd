...
In practice, a map is composed by several layer such, as the bounding boxes, the graticule, the countries of land... All these layers can be loaded in the map design background section by Add Layers.


![](images/RICardo_Design_signs_layers.png)

All layers on the view are positioned in an order that affects their visibility. The top layer will be visible in the foreground.

The position of the layers above and below (foreground or not) can be modified by a simple drag and drop.

Several types of actions can be performed at the layer level, using the following icon-buttons.
...
Drawing the nodes opens a window that allows you to act on several dimensions of their graph

It is possible to  change the style of nodes respecting the cartographic semiology as Bertin (1967). Hereby the general window.
